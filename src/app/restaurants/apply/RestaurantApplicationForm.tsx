...
const formSchema = z.object({
    ...
    companyName: z.string().min(1, "Restaurant name is required"),
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                    ...
                    <input
                        {...register("firstName")}
                        type="text"
                        className="w-full px-4 py-3 rounded-xl bg-brand-800 border border-brand-700 text-white placeholder:text-brand-500/50 outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500/20 transition-all font-sans"
                        placeholder="John"
                    />
                    {errors.firstName && (
            ...
            <div className="space-y-2 pt-2">
                <label htmlFor="revenue" className="block text-sm font-bold text-white font-sans">
                    What is your current annual revenue? <span className="text-orange-500">*</span>
                </label>
                <div className="relative">
                    <select
                        {...register("revenue")}
                        className="w-full px-4 py-4 rounded-xl bg-brand-800 border border-brand-700 text-white outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500/20 transition-all appearance-none font-sans cursor-pointer hover:border-brand-600"
                    >
            ...
            <button
                ...
                className="w-full py-4 mt-6 rounded-xl bg-orange-500 text-white font-bold text-lg hover:bg-orange-600 active:scale-[0.99] transition-all shadow-lg shadow-orange-500/20 flex items-center justify-center gap-2 font-heading disabled:opacity-70 disabled:cursor-not-allowed group"
            >
                {isSubmitting ? (
                    <>
            ...
            <p className="text-center text-xs text-brand-400 font-sans mt-4">
                <CheckCircle2 className="w-3 h-3 inline-block mr-1 text-orange-500" />
